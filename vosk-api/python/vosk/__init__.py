...
class KaldiRecognizer(object):
    def __init__(self, *args):
        if len(args) == 2:
            ...
            self._handle = _c.vosk_recognizer_new_spk(
                args[0]._handle, args[1]._handle, args[2]
            )
        elif len(args) == 3 and type(args[2]) is str:
            self._handle = _c.vosk_recognizer_new_grm(
    ...
    def PartialResult(self):
        return _ffi.string(_c.vosk_recognizer_partial_result(self._handle)).decode(
            "utf-8"
        )
